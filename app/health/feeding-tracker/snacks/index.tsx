...
import { snacksApi } from '@/services/api/feeding/snacks';
...
const SnacksScreen = () => {
  ...
  const handleDeletePress = async (babyId: string, feedingId: string) => {
    Alert.alert(
      "Atıştırmalık Kaydını Sil",
      "Bu atıştırmalık kaydını silmek istediğinizden emin misiniz?",
      [
        { text: "İptal", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              await snacksApi.deleteFeeding(babyId, feedingId);
              fetchBabies();
            } catch (error) {
              Alert.alert('Hata', 'Atıştırmalık kaydı silinirken bir hata oluştu');
            }
          }
        }
      ]
  ...
  const renderFeedingHistory = (snacks: any[], babyId: string) => {
    if (!snacks || snacks.length === 0) {
      return (
        <View style={[styles.emptyState, { backgroundColor: '#FFF' }]}>
          <MaterialIcons name="cookie" size={48} color="#FF69B4" />
          <ThemedText style={styles.emptyStateText}>
            Henüz atıştırmalık kaydı bulunmuyor
          </ThemedText>
        </View>
      );
    }

    return snacks.map((feeding, index) => (
                  ...
                  <View style={styles.profileSection}>
                    ...
                    <View style={styles.babyInfo}>
                      ...
                      <View style={styles.subtitleContainer}>
                        <MaterialIcons name="cookie" size={16} color="#FFF" />
                ...
                <View style={[styles.expandedContent, { backgroundColor: contentBgColor }]}>
                  <View style={styles.buttonContainer}>
                    <TouchableOpacity 
                      ...
                      onPress={() => router.push({
                        pathname: '/health/feeding-tracker/snacks/add',
                    ...
                    <TouchableOpacity 
                      ...
                      onPress={() => router.push({
                        pathname: '/health/feeding-tracker/snacks/stats',
                        params: { babyId: baby.id }
                      })}
                    >
                  ...
                  <View style={styles.feedingList}>
                    {renderFeedingHistory(baby.snacks || [], baby.id)}
                  </View>
                </View>
              )}
            </View>
